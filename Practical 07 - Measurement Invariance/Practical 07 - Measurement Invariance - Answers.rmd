...
In fact, *we already investigated configural measurement invariance* at point
*(b)* by manually fitting the model in each group and checking the global fit
and whether the loadings are significant for both groups. This time, instead of
manually fitting the model to each group, we will pass the list of mean vectors,
...
Our findings do not show support for strong measurement invariance (i.e., the
*strong* measurement invariance model fits significantly worse compared to the
*weak* measurement invariance model). We obtain a $\chi^2$ difference value of
$43.096$ with a $p\text{-value} < 0.001$. Also, the rules of thumbs regarding
the decrease in *CFI* and increase in *RMSEA* show that we do not have support
for strong measurement invariance.
...
lavInspect(model_ex_1_strong_fit, "mu")[[2]] - group_2_means
```
...
do this, we use the `lavTestScore` function in `lavaan`.

First, let us use the `parTable` function in `lavaan` to print the parameter
table and learn the label `lavaan` automatically generated for the intercept of
variable `sim`. Note that, behind the scenes, `lavaan` applies labels
automatically to enforce the equality constraints requested via the argument
`group.equal`.
...
To find the parameter labels, we use check the `plabel` column for the specific
relationship (i.e., `lavaan` equation syntax) that we are interested in. We see
that for the first group, `lavaan` attached the label `.p21.` for the parameter
`sim ~ 1` (i.e., the intercept).
...
looking at the output of `lavTestScore` to understand which \enquote{release of
constraints} results in the highest change in the $\chi^2$ difference, we expect
`.p21. == .p50.` to yield the highest change.
...
argument. Note that, in this case, we are are actually investigating what is
called *strong partial measurement invariance*.
...
# Round fit measures for all models to four decimals.
print(round(fit_measures_all_ex_1, 3))
```

Or print the fit measures as an APA-style table.

```{r size='scriptsize'}
# Print pretty table.
knitr::kable(round(fit_measures_all_ex_1, 3), caption = "Fit measures for all models investigated.")
```
...
# Print the fit measures for all models with three decimals.
print(round(fit_measures_all_ex_1, 3))
```

Or, again, show the measures in an APA-style table.
...
We observe a $\chi^2 = 94.881$ with $DF = 57$ and a $p\text{-value} = 0.001$.
...
context of
reliability](https://mihaiconstantin.shinyapps.io/reliability-measurement-error/)).
Therefore, with strict measurement invariance we ensure that the error part
